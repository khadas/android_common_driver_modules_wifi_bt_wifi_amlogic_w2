...
/**
 * rwnx_fw_trace_save_filters() - Save filters currently configured so that
 * they can be restored with rwnx_fw_trace_restore_filters()
 *
 * @trace: Fw trace control structure
 * @return 0 if filters have been saved and != 0 in case on error
 */
int rwnx_fw_trace_save_filters(struct rwnx_fw_trace *trace)
{
    int i;
    ...
    for (i = 0; i < saved_filters_cnt; i++) {
        saved_filters[i] = rwnx_fw_trace_get_trace_level(&trace->buf, i);
    }
...
/**
 * rwnx_fw_trace_restore_filters() - Restore filters previoulsy saved
 * by rwnx_fw_trace_save_filters()
 *
 * @trace: Fw trace control structure
 * @return 0 if filters have been restored and != 0 in case on error
 */
int rwnx_fw_trace_restore_filters(struct rwnx_fw_trace *trace)
{
    int i;
    ...
    for (i = 0; i < saved_filters_cnt; i++) {
        rwnx_fw_trace_set_trace_level(&trace->buf, i, saved_filters[i]);
    }
