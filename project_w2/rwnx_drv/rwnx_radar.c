 ...
 * Copyright (C) RivieraWaves 2015-2021
 *
 ******************************************************************************
 */
...
bool pulse_queue_enqueue(struct pri_detector *pde, u64 ts)
{
    ...
    /* coverity[leaked_storage] - variable "p" was add and free in list */
    return true;
}
                                ...
                                u64 ts, u32 min_count)
{
    ...
    list_for_each_entry(p, &pde->pulses, head) {
        ...
        /* coverity[leaked_storage] - variable "ps" was add and free in list */
    }
    return true;
}
...
{
#ifdef CONFIG_RWNX_SOFTMAC
    ieee80211_radar_detected(rwnx_hw->hw);
#else
...
static void rwnx_radar_process_pulse(struct work_struct *ws)
{
    ...
    enum rwnx_radar_chain chain;
    u32 pulses[RWNX_RADAR_LAST][RWNX_RADAR_PULSE_MAX] = {{0}};
                                 ...
                                 struct pri_detector *pde)
{
    ...
    if (buf == NULL) {
        ...
        return (sizeof("Freq = %3.dMhz\n") + nb_seq * sizeof(seq_info));
    }

    res = scnprintf(buf, len, "Freq = %3.dMhz\n", pde->freq);
    write += res;
    len -= res;
                                     ...
                                     struct rwnx_radar *radar, u8 chain)
{
    ...
    /* */
    for (i = 0; i < dpd->num_radar_types; i++) {
        res = scnprintf(&buf[write], len, "Type = %3.d\n", i);
                                   ...
                                   struct rwnx_radar *radar, u8 chain)
{
    ...
    if (buf == NULL) {
        return (count * sizeof("2001/02/02 - 02:20 5126MHz\n")) + 1;
     }
