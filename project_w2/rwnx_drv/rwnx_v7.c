...
int rwnx_v7_platform_init(struct pci_dev *pci_dev, struct rwnx_plat_pci **rwnx_plat_pci)
{
    ...
    *rwnx_plat_pci = kzalloc(sizeof(struct rwnx_plat_pci) + sizeof(struct rwnx_v7),
    ...
    g_pcie_bar0_base_addr = (unsigned long)rwnx_v7->pci_bar0_vaddr;
    ...
    printk("%s:%d, bar2 0x%lx\n", __func__, __LINE__, (unsigned long)rwnx_v7->pci_bar2_vaddr);
    ...
    printk("%s:%d, bar4 0x%lx\n", __func__, __LINE__, (unsigned long)rwnx_v7->pci_bar4_vaddr);

    ipc_basic_address = rwnx_v7->pci_bar4_vaddr + PCIE_BAR4_TABLE2_OFFSET; // bar4 table2 0x60800000
    (*rwnx_plat_pci)->deinit = rwnx_v7_platform_deinit;
