...
#else /* i.e. #ifdef CONFIG_RWNX_FULLMAC */
/**
...
#define RWNX_TXQ_CLEANUP_INTERVAL (10 * HZ) //10s in jiffies
#define RWNX_TXQ_MAX_QUEUE_JIFFIES (20 * HZ)
...
struct rwnx_txq {
    ...
    struct rwnx_sw_txhdr *amsdu_anchor;
...
static inline bool rwnx_txq_is_ps(struct rwnx_txq *txq)
{
    return (txq->status & RWNX_TXQ_STOP_STA_PS);
}
...
{
    return ((!rwnx_txq_is_stopped(txq) || txq->push_limit) &&
...
#elif defined(CONFIG_RWNX_SOFTMAC)
#define foreach_sta_txq(sta, txq, tid, rwnx_hw)                         \
...
#define foreach_sta_txq_prio(sta, txq, tid, i, rwnx_hw)                 \
    for (i = 0, tid = nx_tid_prio[0], txq = rwnx_txq_sta_get(sta, tid); \
         i < NX_NB_TXQ_PER_STA;                                         \
         i++, tid = nx_tid_prio[i % NX_NB_TXQ_PER_STA], txq = rwnx_txq_sta_get(sta, tid))
#else /* CONFIG_RWNX_FULLMAC */
#define foreach_sta_txq_prio(sta, txq, tid, i, rwnx_hw)                          \
    for (i = 0, tid = nx_tid_prio[0], txq = rwnx_txq_sta_get(sta, tid, rwnx_hw); \
         i < NX_NB_TXQ_PER_STA;                                                  \
         i++, tid = nx_tid_prio[i % NX_NB_TXQ_PER_STA], txq = rwnx_txq_sta_get(sta, tid, rwnx_hw))
#endif
