...
/**
 * aml_mu_group_sta_init - Initialize group information for a STA
 *
                            ...
                            const struct ieee80211_vht_cap *vht_cap)
{
    ...
    if (!vht_cap ||
        !(vht_cap->vht_cap_info & IEEE80211_VHT_CAP_MU_BEAMFORMEE_CAPABLE)) {
            sta->group_info.map = AML_SU_GROUP;
    }
}

/**
 * aml_mu_group_sta_del - Remove a sta from all MU group
 *
...
void aml_mu_group_sta_del(struct aml_hw *aml_hw, struct aml_sta *sta)
{
    ...
    group_sta_for_each(sta, group_id, map) {
        struct aml_mu_group *group = aml_mu_group_from_id(mu, group_id);
...
/**
 * aml_mu_group_sta_get_map - Get the list of group a STA belongs to
 *
...
/**
 * aml_mu_group_sta_get_pos - Get sta position in a group
 *
 * @aml_hw: main driver data
 * @sta: pointer to the sta
 * @group_id: Group id
                              ...
                              int group_id)
{
    ...
    group = aml_mu_group_from_id(&aml_hw->mu, group_id);
    if (!group)
        return -1;
...
/**
 * aml_mu_group_move_head - Move (or add) one element at the top of a list
 *
 * @list: list pointer
 ...
 */
static inline
void aml_mu_group_move_head(struct list_head *list, struct list_head *elem)
{
...
/**
 * aml_mu_group_remove_users - Remove all the users of a group
 *
 ...
 */
static inline
void aml_mu_group_remove_users(struct aml_mu_info *mu,
                                struct aml_mu_group *group)
{
    struct aml_sta *sta;
    ...
    for (i = 0; i < CONFIG_USER_MAX; i++) {
        if (group->users[i]) {
            ...
            aml_mu_group_move_head(&mu->update_sta,
...
/**
 * aml_mu_group_add_users - Add users to a group
 *
 * @mu: pointer on MU info
 ...
 */
static inline
void aml_mu_group_add_users(struct aml_mu_info *mu,
                             struct aml_mu_group *group,
                             int nb_user, struct aml_sta **users)
{
...
/**
 * aml_mu_group_create_one - create on group with a specific group of user
 *
 * @mu: pointer on MU info
 ...
 */
static
int aml_mu_group_create_one(struct aml_mu_info *mu, int nb_user,
                             struct aml_sta **users, int *nb_group_left)
{
    int i, group_id;
    struct aml_mu_group *group;
    u64 group_match;
    ...
    if (group_match) {
        ...
        group_id = AML_GET_FIRST_GROUP_ID(group_match);
        group = aml_mu_group_from_id(mu, group_id);
        aml_mu_group_move_head(&mu->active_groups, &group->list);
        return 0;
    }
    ...
    if (group_avail) {
        ...
        struct aml_sta *users2[CONFIG_USER_MAX];
        int nb_user2;
        ...
        group_for_each(group_id, group_avail) {
            ...
            if ((group->user_cnt + nb_user2) <= CONFIG_USER_MAX) {
                aml_mu_group_add_users(mu, group, nb_user2, users2);
                aml_mu_group_move_head(&mu->active_groups, &group->list);
                return 0;
            }
        }
    ...
    /* create a new group */
    group = list_last_entry(&mu->active_groups, struct aml_mu_group, list);
    aml_mu_group_remove_users(mu, group);
    aml_mu_group_add_users(mu, group, nb_user, users);
    aml_mu_group_move_head(&mu->active_groups, &group->list);
    (*nb_group_left)--;
...
/**
 * aml_mu_group_create - Create new groups containing one specific sta
 *
 * @mu: pointer on MU info
 ...
 */
static
void aml_mu_group_create(struct aml_mu_info *mu, struct aml_sta *sta,
                          int *nb_group_left)
{
    struct aml_sta *user_sta = sta;
    ...
    while (*nb_group_left) {
        ...
        if (nb_user > 1) {
            if (aml_mu_group_create_one(mu, nb_user, users, nb_group_left))
                (*nb_group_left)--;
...
/**
 * aml_mu_group_work - process function of the "group_work"
 *
 ...
 */
void aml_mu_group_work(struct work_struct *ws)
{
    struct delayed_work *dw = container_of(ws, struct delayed_work, work);
    struct aml_mu_info *mu = container_of(dw, struct aml_mu_info, group_work);
    struct aml_hw *aml_hw = container_of(mu, struct aml_hw, mu);
    ...
    list_for_each_entry_safe(sta, next, &mu->active_sta, group_info.active) {
        if (nb_group_left)
            aml_mu_group_create(mu, sta, &nb_group_left);
    ...
    if (! list_empty(&mu->update_sta)) {
        list_for_each_entry_safe(sta, next, &mu->update_sta, group_info.update) {
            aml_send_mu_group_update_req(aml_hw, sta);
            list_del(&sta->group_info.update);
        }
    }
...
/**
 * aml_mu_group_init - Initialize MU groups
 *
 * @aml_hw: main driver data
 *
...
void aml_mu_group_init(struct aml_hw *aml_hw)
{
    ...
    mu->group_cnt = 0;
    mu->next_group_select = jiffies;
    INIT_DELAYED_WORK(&mu->group_work, aml_mu_group_work);
    sema_init(&mu->lock, 1);
}

/**
 * aml_mu_set_active_sta - mark a STA as active
 *
 * @aml_hw: main driver data
 * @sta: pointer to the sta
                            ...
                            int traffic)
{
    ...
    if (!sta || (sta->group_info.map & AML_SU_GROUP))
        return;
    ...
    if ((sta->group_info.last_update != mu->update_count) ||
        ...
        aml_mu_group_move_head(&mu->active_sta, &sta->group_info.active);

        if (!delayed_work_pending(&mu->group_work) &&
            !list_is_singular(&mu->active_sta)) {
            schedule_delayed_work(&mu->group_work,
                                  msecs_to_jiffies(AML_MU_GROUP_INTERVAL));
        }
    }
}

/**
 * aml_mu_set_active_group - mark a MU group as active
 *
 * @aml_hw: main driver data
 * @group_id: Group id
 *
...
/**
 * aml_mu_group_sta_select - Select the best group for MU stas
 *
 * @aml_hw: main driver data
 *
 ...
 * for each group for the previous @AML_MU_GROUP_SELECT_INTERVAL interval:
...
void aml_mu_group_sta_select(struct aml_hw *aml_hw)
{
    ...
    list_for_each_entry(vif, &aml_hw->vifs, list) {
        ...
        list_for_each_entry(sta, head, list) {
            ...
            if (sta->group_info.cnt == 0 ||
                sta_traffic < AML_MU_GROUP_MIN_TRAFFIC)
                continue;
    ...
    mu->next_group_select = jiffies +
        msecs_to_jiffies(AML_MU_GROUP_SELECT_INTERVAL);
