 ...
 */
void aml_chanctx_link(struct aml_vif *vif, u8 ch_idx,
                       struct cfg80211_chan_def *chandef)
{
    ...
    AML_INFO("vif:%d ch_id:%d", vif->vif_index, ch_idx);
    vif->ch_index = ch_idx;
    ...
    if (chandef) {
        AML_INFO("chan freq[cfreq1:%d cfreq2:%d]", chandef->center_freq1, chandef->center_freq2);
...
static int aml_open(struct net_device *dev)
{
    ...
    AML_INFO("vif index=%d, type=%d\n",
            add_if_cfm.inst_nbr, AML_VIF_TYPE(aml_vif));
                                               ...
                                               struct vif_params *params)
{
    ...
    ndev = alloc_netdev_mqs(sizeof(*vif), name, name_assign_type,
                            aml_netdev_setup, NX_NB_NDEV_TXQ, 4);
    if (!ndev)
        return NULL;
...
{
    struct aml_hw *aml_hw = wiphy_priv(wiphy);
    struct aml_vif *aml_vif = container_of(request->wdev, struct aml_vif,
                                             wdev);
    int error;
 ...
 */
static int aml_cfg80211_del_station(struct wiphy *wiphy,
                                     struct net_device *dev,
 ...
 */
static int aml_cfg80211_start_ap(struct wiphy *wiphy, struct net_device *dev,
                                  struct cfg80211_ap_settings *settings)
{
    ...
    switch (apm_start_cfm.status)
    {
        case CO_OK:
        {
            ...
            aml_mu_group_sta_init(sta, NULL);
            spin_lock_bh(&aml_hw->cb_lock);
            aml_chanctx_link(aml_vif, apm_start_cfm.ch_idx,
                              &settings->chandef);
    ...
    if (error) {
        ...
        aml_scc_init();
        aml_scc_check_chan_confilct(aml_hw);
#endif
    }
    if (vif != NULL) {
        vif->sta.scan_hang = 0;
                                ...
                                unsigned int duration, u64 *cookie)
{
    ...
    if (aml_hw->roc) {
        AML_INFO("cancel roc as launched %d\n");
        return -EBUSY;
    }
                                 ...
                                 u64 *cookie)
{
    ...
    if (offchan) {
        ...
        if (aml_hw->roc) {
            ...
            if ((aml_hw->roc->vif != aml_vif) ||
                (aml_hw->roc->chan->center_freq != params->chan->center_freq))
                return -EINVAL;
                                  ...
                                  struct station_info *sinfo)
{
    ...
    // Generic info
    rx_vect1->rssi1 = AML_REG_READ(aml_plat, AML_ADDR_MAC_PHY, REG_OF_BEACON_RSSI);
    ...
    list_for_each_entry(aml_vif, &aml_hw->vifs, list) {
        ...
        if (iftype == NL80211_IFTYPE_STATION) {
            ...
            if (aml_vif->sta.ap && aml_vif->sta.ap->valid) {
                ...
                while (!aml_txq_is_empty(aml_hw, aml_vif->sta.ap)) {
                    msleep(10);
                    if (count++ > 10) {
        ...
        if (iftype == NL80211_IFTYPE_AP) {
            ...
            while (!aml_unktxq_is_empty(aml_vif)) {
                msleep(10);
                if (count++ > 10) {
                    AML_INFO("ap:unktxq is not empty\n");
                    goto err;
                }
            }
            list_for_each_entry(sta, &aml_vif->ap.sta_list, list) {
                while (!aml_txq_is_empty(aml_hw, sta)) {
                    msleep(10);
                    if (count++ > 10) {
                        AML_INFO("ap:txq is not empty\n");
                        goto err;
                    }
                }
            }
...
unsigned char aml_parse_cali_param(char *varbuf, int len, struct Cali_Param *cali_param)
{
    ...
    aml_get_s8_item(varbuf, len, "wf5g_hesu20_tpwr", &cali_param->wf5g_he20_tpwr[0]);
    aml_get_s8_item(varbuf, len, "wf5g_hesu40_tpwr", &cali_param->wf5g_he40_tpwr[0]);
    aml_get_s8_item(varbuf, len, "wf5g_hesu80_tpwr", &cali_param->wf5g_he80_tpwr[0]);
...
static int aml_readFile(struct file *fp, char *buf, int len)
{
    int rlen = 0, sum = 0;
...
*/
int aml_isFileReadable(const char *path, u32 *sz)
{
    struct file *fp;
    int ret = 0;
    char buf;

    fp = FILE_OPEN(path, O_RDONLY, 0);
    if (IS_ERR(fp)) {
        ret = PTR_ERR(fp);
    } else {
        if (1 != aml_readFile(fp, &buf, 1)) {
            ret = PTR_ERR(fp);
        }
...
unsigned char aml_get_cali_param(struct Cali_Param *cali_param)
{
    ...
    if (aml_isFileReadable(chip_id_buf, NULL) != 0) {
    ...
    fp = FILE_OPEN(chip_id_buf, O_RDONLY, 0);
    ...
    error = FILE_STAT(chip_id_buf, &stat);
    if (error) {
        FILE_CLOSE(fp, NULL);
        goto err;
    }
    ...
    if (FILE_READ(fp, content, size, &fp->f_pos) != size) {
        FREE(content, "wifi_cali_param");
        FILE_CLOSE(fp, NULL);
        goto err;
    }

    len = aml_process_cali_content(content, size);
    aml_parse_cali_param(content, len, cali_param);
    if (chip_id_l & 0xf0) {
        if (chip_id_l & BIT(5))
            cali_param->rf_num = 2;
        else
...
void aml_config_cali_param(struct aml_hw *aml_hw)
{
    ...
    err = aml_get_cali_param(&cali_param);

    if (err == 0) {
        printk("%s:%d, set calibration parameter \n", __func__, __LINE__);
        aml_set_cali_param_req(aml_hw, &cali_param);
    }
    else {
...
#ifdef SCC_STA_SOFTAP
                aml_scc_check_chan_confilct(aml_vif->aml_hw);
#endif
...
int aml_cfg80211_init(struct aml_plat *aml_plat, void **platform_data)
{
    ...
    aml_hw->rxbufs  = (struct aml_ipc_buf *)aml_prealloc_get(PREALLOC_BUF_TYPE_RX, sizeof(struct aml_ipc_buf) * AML_RXBUFF_MAX, &out_size);
    aml_hw->txq  = (struct aml_txq *)aml_prealloc_get(PREALLOC_BUF_TYPE_TXQ, sizeof(struct aml_txq) * NX_NB_TXQ, &out_size);
#endif
    aml_hw->wiphy = wiphy;
    ...
    *platform_data = aml_hw;
    ...
    wiphy_info(wiphy, "New interface create %s", wdev->netdev->name);
    aml_sync_trace_timer_attach(aml_hw);

    if (aml_bus_type == PCIE_MODE) {
        aml_cfgfile_update_rps();
    }
...
void aml_cfg80211_deinit(struct aml_hw *aml_hw)
{
    ...
    aml_sync_trace_timer_cancel(aml_hw);
