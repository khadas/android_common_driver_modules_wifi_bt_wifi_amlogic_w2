 ...
 * HW type
 ******************************************************************************/
#define AML_MACHW_NX 1
#define AML_MACHW_HE 2
...
extern const struct aml_legrate legrates_lut[];
...
struct tx_policy_tbl {
    ...
    union aml_pol_phy_ctrl_info_1 phyctrlinfo_1;
    /* PHY Control 2 Information used by MAC HW */
    union aml_pol_phy_ctrl_info_2 phyctrlinfo_2;
    /* MAC Control 1 Information used by MAC HW */
    union aml_pol_mac_ctrl_info_1 macctrlinfo_1;
    /* MAC Control 2 Information used by MAC HW */
    union aml_pol_mac_ctrl_info_2 macctrlinfo_2;

    union aml_rate_ctrl_info  ratectrlinfos[NX_TX_MAX_RATES];
    struct aml_power_ctrl_info powerctrlinfos[NX_TX_MAX_RATES];
};

#ifdef CONFIG_AML_SOFTMAC

/* c.f RW-WLAN-nX-MAC-HW-UM */
union aml_hw_txstatus {
    struct {
...
#define AML_WD_IS_AMPDU(whichdesc) \
    (((whichdesc) & __WD_AMPDU_BAPOL) == __WD_AMPDU_BAPOL)
#define AML_WD_IS_FIRST(whichdesc) \
    ((whichdesc) == __WD_AMPDU_FIRST)
#define AML_WD_IS_LAST(whichdesc) \
    ((whichdesc) == __WD_AMPDU_LAST)

/* c.f RW-WLAN-nX-MAC-HW-UM */
union aml_thd_phy_ctrl_info {
    struct {
        u32 soundingTx       : 1;
        u32 smoothingTx      : 1;
...
/**
 * struct aml_hw_txhdr - Hardware part of tx header
 *
...
struct aml_hw_txhdr {
    ...
    union aml_thd_mac_ctrl_info_1 mac_ctrl_info;
    union aml_thd_phy_ctrl_info phy_ctrl_info;
};
...
#define AML_RX_HD_NX_DECR_UNENC           0 // Frame unencrypted
#define AML_RX_HD_NX_DECR_ICVFAIL         1 // WEP/TKIP ICV failure
#define AML_RX_HD_NX_DECR_CCMPFAIL        2 // CCMP failure
#define AML_RX_HD_NX_DECR_AMSDUDISCARD    3 // A-MSDU discarded at HW
#define AML_RX_HD_NX_DECR_NULLKEY         4 // NULL key found
#define AML_RX_HD_NX_DECR_WEPSUCCESS      5 // Security type WEP
#define AML_RX_HD_NX_DECR_TKIPSUCCESS     6 // Security type TKIP
#define AML_RX_HD_NX_DECR_CCMPSUCCESS     7 // Security type CCMP (or WPI)

/* Decryption status subfields. (HE hardware) */
#define AML_RX_HD_DECR_UNENC     0 // Frame unencrypted
#define AML_RX_HD_DECR_WEP       1 // Security type WEP
#define AML_RX_HD_DECR_TKIP      2 // Security type TKIP
#define AML_RX_HD_DECR_CCMP128   3 // Security type CCMP (128 bits)
#define AML_RX_HD_DECR_CCMP256   4 // Security type CCMP (256 bits)
#define AML_RX_HD_DECR_GCMP128   5 // Security type GCMP (128 bits)
#define AML_RX_HD_DECR_GCMP256   6 // Security type GCMP (256 bits)
#define AML_RX_HD_DECR_WAPI      7 // Security type WAPI
#define AML_RX_HD_DECR_NULLKEY  15 // NULL key found
...
int aml_machw_type(uint32_t machw_version_2);
void aml_rx_vector_convert(int machw_type, struct rx_vector_1 *rx_vect1,
                            struct rx_vector_2 *rx_vect2);
void aml_rx_status_convert(int machw_type, struct mpdu_status *status);
